...
## Checkpoint reach (0.6.25)

The delegation freezes the unit's checkpoint from the server's spec: its `Files:` and every `Test:` line, digested onto the attempt. A `go [-C dir] test` selection that omits the conventional owning package of an authorized file (a `.go` file's directory; a `testdata/**` fixture's parent package) refuses the delegation (`CHECKPOINT REACH`) unless the owner overrides, recorded as `reach_override`; `preflight_check` reports the same and a reach-only failure is the one preflight failure the override may consume. `set_verdict pass` refuses when the spec's definition moved (`CHECKPOINT CHANGED`) or when the guard's frozen authorized set is wider than the frozen selection covers. Only `go test` selectors are understood; an opaque clause (any other runner, a bare import path, `go test -c`, a pipe) blocks every refusal, filters are reported, files with no owning package are reported. What this establishes: the checkpoint omits a package. What it does not: that the selected tests observe the change. A direct-fix attempt is bound by the newest delegation's frozen obligations (contract, references, forward, checkpoint).

## Deliverables, forward declarations and evidence identity (0.6.24)

The `foreman-contract` block is strict (an unknown field refuses it) and may declare `deliverables`: files the smoke plan produces, each with a closed assertion set (`min_bytes`, `max_bytes`, `json_nonempty_path`, a bounded `json_array_length`, `values_in { path, reference }` against a repo file of allowed values). The delegation freezes the contract digest and every reference digest; `set_verdict pass` checks the frozen digest before any exemption (`CONTRACT CHANGED`), then requires, wherever a plan or deliverable is declared in ANY phase, a passing `live_smoke` for the attempt whose harness, input, deliverable and reference digests still match (`SMOKE REQUIRED`). `live_smoke` refuses while a declared deliverable already exists, so what it digests is what the run produced. Every digest covers complete bytes; an inventory past the bound is refused. A `record_review` that can carry the gate cites the newest passing receipt of every covered unit with deliverables in `smoke_receipts`; the gate refuses an uncited receipt (`DELIVERABLES`). `preflight_check creates: [{ file, tests }]` makes citations of not-yet-existing files forward; the promise rides the delegation and `set_verdict pass` refuses until each test is declared in its file (`FORWARD CITATIONS UNMET`). `-run` operands resolve as Go regexes against test declarations. `add_rejection.ts` defaults to server time. What a deliverable receipt proves: Foreman executed the frozen plan, these bytes appeared during the run, and they satisfied the declared properties. What it does not prove: that the properties are the right ones, or that the harness exercises production code; those stay with review.

## Reality as a source of truth (0.6.22)

A unit's external claims and its live smoke plan are authored once, in a `foreman-contract` fenced JSON block under the unit's heading in `Docs/spec.md`, and executed only from there. `contract_probe { phase, unit_id, claim_id }` loads the request recipe and assertions from the claim and records the claim id and the contract digest; a url-mode probe is recorded `diagnostic` and never satisfies a claim. `${ENV:NAME}` header values and a smoke plan's env names resolve from the server environment first and `~/.foreman-mcp/.env` second (0.6.23; the same precedence as the worker key), are registered for redaction, and appear in output as name and source only. Capture streams under a 4 MB cap and an incomplete capture fails every body assertion. `live_smoke { phase, unit_id, plan_id }` runs the registered plan through `run_tests` (no shell, the runner allowlist applies) in the plan's cwd and records a receipt bound to the attempt, the contract digest, a digest of the harness files and a digest of the application inputs, all server-computed. In a `has_api` phase: `preflight_check` requires a passing claim-mode probe for every registered claim under the current digest (no block is a refusal; `claims: [], smoke: null` is the reviewed opt-out); the delegation freezes the digest and refuses a receipt taken under another; `set_verdict v:'pass'` recomputes the digests and refuses (`SMOKE REQUIRED`) without a current passing smoke whose digests still match, waivable by `user_override` as `cap_override.waived: smoke`. The ledger keeps one repository window per root: a guard snapshot acquires it (`editing`), an ok comparison moves it to `validation`, any verdict, `close_attempt` on the owning attempt, or a new attempt on the same unit releases it, and another unit's delegation or snapshot is refused meanwhile (`WINDOW BUSY`). A guard comparison carries the baseline hash it was taken against and is refused against a different baseline. Servers started without a spec path keep the 0.6.21 behaviour. What a smoke receipt proves: Foreman executed the frozen recipe on these bytes and saw this result. What it does not prove: that the harness honestly exercises production code; the harness inventory exists so review can read it.
