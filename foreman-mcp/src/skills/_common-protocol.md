...
5. Read only the bounded ledger slice needed for the current action, e.g. `read_ledger({ query: "verdicts", phase: "<current-phase>", limit: 50 })` or `read_ledger({ phase, unit_id })`. Never start a session with `query:"full"`. Table cells clip at 240 chars — a unit's full verdict note comes from `read_ledger({ phase, unit_id })`; full rejection/review text from `read_ledger({ query: "full", phase })`.
