...
- **Contract block (every unit in a phase that talks to an external system):** a ```foreman-contract fenced JSON block under the unit's heading: `{ "unit", "claims": [{ "id", "text", "request": { "method", "url", "headers": { "authorization": "Bearer ${ENV:NAME}" } }, "assertions": { "status", "json_nonempty_path", "json_array_length": { "path", "max" } } }], "smoke": { "id", "runner", "args", "cwd", "env": [NAMES], "harness_files", "input_files", "checks": { "exit_code", "stdout_contains" } } }`. One claim per fact the unit depends on, with the request that proves it and an assertion a 200 with zero rows fails; the smoke plan runs the unit's OWN transport path against the real system through the project's runner, `harness_files` lists every file whose content decides its result, `input_files` the application code it exercises. Add `"deliverables": [{ "id", "path", "assertions": { "max_bytes", "json_array_length": { "path", "min" }, "values_in": { "path": "rows[].country", "reference": "Docs/allowed-countries.txt" } } }]` for every file the plan produces: name the property that would have caught the wrong output (a size bound, a row count, membership in a reference file you commit alongside the spec and that review owns), because Foreman evaluates only what is declared and a deliverable with no observable property is refused. For every accepted domain (codes, enums, ids) commit the reference file and name two values outside it in the unit's test directive. Write `"smoke": null` and `"deliverables": []` only as reviewed statements that the unit touches nothing external and emits nothing to observe; omit the block only for units in phases without `has_api`. Foreman executes claims and plans from this block alone (`contract_probe` by claim id, `live_smoke` by plan id), so a claim you cannot write a request for is a discovery gap: run the Probe check first.
