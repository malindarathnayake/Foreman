...
`.foreman-progress.json` holds a per-unit status and note for the checklist. It is descriptive. `session_orient` compares it to the ledger and reports `state_drift` when they disagree, and the procedure says: never choose the progress target over the ledger target.
