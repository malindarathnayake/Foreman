...
6. **Classify** each finding `confirmed`, `rejected`, or `unverified` by checking it against the code. A seat with zero findings and no examined list is `completion: "partial"`, never clean. If one seat confirmed findings and another reported nothing, the model may re-prompt the silent seat once, naming only the files involved, and records that pass as `stage: "cross_exam"`. It never counts as a second independent seat.
