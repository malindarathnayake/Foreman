...
DELEGATION CAP: unit 'u3' has 3 failed attempts since its last pass (cap 3). A further delegation needs the owner's decision: record it once with authorize_attempts { attempts, reason, user_override: true } ... or set data.user_override: true on this write ...
AUTHORIZE BLOCKED: unit 'u3' has 1 failed attempt(s) since its last pass (cap 3). Below the cap attempts need no authorization; a grant issued early would defeat the cap.
