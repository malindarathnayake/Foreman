...
| `FOREMAN_AIDER_PYTHON`, `FOREMAN_AIDER_HARNESS` | Interpreter and harness script for `aider_worker` | `python3` then `python`; the bundled harness |
| `FOREMAN_AIDER_WORKTREE_ROOT`, `FOREMAN_AIDER_WORKTREE_MAX_AGE_MS` | Where detached worktrees are created and when orphans are reclaimed | `Docs/.foreman-worktrees`, see tool description |
