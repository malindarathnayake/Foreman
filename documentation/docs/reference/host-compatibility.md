...
Top can reuse a bounded native worker for fixes and test changes, use compact follow-ups and focused intermediate checks, and obtain independent verification of the delta against retained review coverage. Middle gets mechanical worker reuse and compact follow-ups with normal checks/review. Native reuse needs an actual worker ID in the same active session, recorded at the verdict or bound by the first correction while the attempt carries no ID; an unsupported follow-up mechanism or a host switch requires a fresh worker. Existing sidecar `invoke_worker` attempts do not qualify for native reuse.
